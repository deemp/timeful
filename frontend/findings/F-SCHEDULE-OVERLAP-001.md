...
- Verification evidence: `npm run lint`, `npm run typecheck`, `npm run build`, and `npm run test:unit` passed in `frontend/`. Added `src/composables/schedule_overlap/scheduleOverlapStorage.test.ts`. Firefox inspection of `event-respondents-panel` passed against the frontend on `http://127.0.0.1:4173`.
